...
class PartialHead(nn.Module):
    """ 
    A head that operates on a smaller input dimension (head_size) 
    rather than the full n_embed.
    """
    def __init__(self, head_size):
        super().__init__()
    ...
    def forward(self, x):
        b,t,c = x.shape # c is head_size here
        k = self.key(x)
        q = self.query(x)
        # compute attention scores
        wei = q @ k.transpose(-2,-1) * c**-0.5
...
class PartialMultiheadAttention(nn.Module):
    def __init__(self, num_heads, head_size):
        super().__init__()
        self.heads = nn.ModuleList([PartialHead(head_size) for _ in range(num_heads)])
        self.proj = nn.Linear(n_embed, n_embed)
        self.dropout = nn.Dropout(dropout)
    def forward(self, x):
        # Break apart the input into num_heads of head_size chunks
        x_chunks = torch.chunk(x, len(self.heads), dim=-1)
        out = torch.cat([h(chunk) for h, chunk in zip(self.heads, x_chunks)], dim=-1)
        out = self.dropout(self.proj(out))
        return out
...
class Block(nn.Module):
    def __init__(self, n_embed, n_head, use_partial=False):
        super().__init__()
        head_size = n_embed // n_head
        if use_partial:
            self.sa = PartialMultiheadAttention(n_head, head_size)
        else:
...
class Transformer(nn.Module):
    ...
    def __init__(self):
        ...
        # Alternate between standard MultiheadAttention and the new Partial version
        self.blocks = nn.Sequential(*[
            Block(n_embed, n_head=n_head, use_partial=(i % 2 == 1)) for i in range(n_layer)
        ])
